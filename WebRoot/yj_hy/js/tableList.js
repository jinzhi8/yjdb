...
layui.use(['form', 'layer', 'laydate', 'table', 'laytpl', 'jquery', 'upload'], function () {
    ...
    function addDbj() {
        if(type=="4") {
            top.addTab($('<a data-url="' + getRootPath() + '/yj_hy/tableAddMsss.jsp?type=' + type + '&status=' + status + '"><i class="layui-icon" data-icon=""></i><cite>新增办件</cite></a>'), function () {
                reload();
            });
        }else{
    ...
    function addNews(edit) {
        if(type=="4") {
            top.addTab($('<a data-url="' + getRootPath() + '/yj_hy/tableAddMsss.jsp?unid=' + edit.unid + '&type=' + type + '&status=' + status + '"><i class="layui-icon" data-icon=""></i><cite>编辑办件</cite></a>'), function () {
                reload();
            });
        }else{
