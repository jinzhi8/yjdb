...
//
    ...
    //alert (year)
    //alert (month)
    document.all['zdays'].value=days
    document.all['zmonth'].value=month
    document.all['zyear'].value=year
    ...
    function drawCal(firstDay, lastDate, monthName, year) {
            ...
            var colWidth = 72 // width of columns in table   92
            var dayCellHeight = 10   // height of cells containing days of the week
            var dayColor = "darkblue" // color of font representing week days
            var cellHeight = 72  // height of cells representing dates in the calendar
            var todayColor = "red" // color specifying today's date in the calendar
            var timeColor = "purple" // color of font representing current time

            var text = ""
            text += '<CENTER>'
            text += '<TABLE BORDER=1 style="font-size: 9pt" width=100% cellspacing=2 cellpadding=0 bordercolordark=#ffffff bordercolorlight=#999999 >'
            text += '<TH bgcolor="#ebebeb" COLSPAN=7 HEIGHT=' + headerHeight + '>'
            text += '<a href=arrange.jsp?year='+lastyear+'&month='+lastmonth+'&share='+share+'>'
            text+='<img src=../resources/images/arrange/last.gif border=0 width=6 height=9></a>&nbsp;'+year+"年"+monthName + '&nbsp;'
            text+='<a href=arrange.jsp?year='+nextyear+'&month='+nextmonth+'&share='+share+'><img border=0 src=../resources/images/arrange/next.gif width=6 height=9></a>'
            text += '</TH>' // close header cell

        // variables to hold constant settings
            var openCol = '<TD bgcolor="#e4e4e4" WIDTH=' + colWidth + ' HEIGHT=' + dayCellHeight + '>'
        ...
        // create first row of table to set column width and specify week day
            text += '<TR ALIGN="center" VALIGN="middle"  style="font-size: 9pt">'
...
for (var row = 1; row
    <= Math.ceil((lastDate + firstDay - 1) / 7); ++row) {
            text += '<TR bgcolor="#ebeaf2" ALIGN="right" VALIGN="top" style="font-size: 9pt">'
    ...
    break
        ...
        //***************************显示显示次序/start
        if (solarFestival!="")
        {kingfort = "<font color=blue><b>" + solarFestival + "</b></font>"
                }
        ...
        //***************************显示显示次序/end
        //var kingfort=cDay(cld[digit-1].lDay)//日期下显示的内容

        //text += '
            <TD HEIGHT=' + cellHeight + '>
                <div id="d'+digit
				//amonth=(month.length==1)?"0"+month:month;
                ...
                if(digit==document.all.adate.value)
                {
                        text += '<TD HEIGHT=' + cellHeight + ' bgcolor="#B8C3DE">'
                        }else{
                        text += '<TD HEIGHT=' + cellHeight + '>'
                        }

                text += '
                    <div id="d'+year+'-'+amonth+'-'+adigit
				text+='">
                        <a href="arrange.jsp?year='+year+'&month='+month+'&day='+digit+'&share='+share+'" >
                            <font size=5 color='+showcolor+'>' + digit + '</font>
                            <br>'+kingfort+'</a>
                        </div>
                    </TD>
                '
...
for (var row = 1; row
    ...
    break
        ...
        < firstDay)
        {
                ...
                text += '<TD HEIGHT=16><a href="#" >' + digit + '</a></TD>'
                digit++
                }
        }
        text += '
    ...
    function printYearMon()
    {
            ...
            tables+='<a href=arrange.jsp?year='+ (year-1) +'&share='+share+'>'
            tables+='<img src=../resources/images/arrange/last.gif width=6 height=9 border=0></a>&nbsp;&nbsp;&nbsp;'+year+'年&nbsp;&nbsp;&nbsp;'
            tables+='<a href=arrange.jsp?year='+(year+1)+'&share='+share+'><img src=../resources/images/arrange/next.gif width=6 height=9 border=0></a>'
            tables+='</td></tr>'
...
function printDayTodo() {
    var ls = '<a href="javascript:;" onclick=javascript:window.open("arrangeform.jsp?year=' + year + '&month=' + month + '&day=' + day + '&share=' + share + '&bgh='
    var showStr = '您好，您';
    if (share != '0') {
        showStr = '';
    }
    tables = showStr + '<font color=#FF0000>' + year + '年' + month + '月' + day + '日</font>' + '的安排如下：<table cellspacing=0 bordercolordark=#ffffff cellpadding=2 width=99% bordercolorlight=#999999 border=1 align="center">'
    tables += '<tr bgcolor=#dddddd height=10px><td align=middle width=42>&nbsp;</td><td align=middle width="182">'
    tables += ls + '08","","scrollbars=no,titlebar=yes,toolbar=no,menubar=no,resizebar=yes,width=470,height=340,top=200,left=200");return false;><span class="viewbutton">工作安排</span></a></td></tr>'

    for (var i = 1; i <= 15; i++) {
        //if (i<10)
        //{
        tables += '<tr bgcolor=#dddddd height=10px><td align=middle width=42>'
        tables += i + '&nbsp;</td><td width="182" align=left valign=middle><div id="t' + i + '">&nbsp;</div></td></tr>'
    ...
    //tables+='<tr bgcolor=#dddddd height=10px><td align=middle width=42>'+ls+'>其它</a></td><td width="182"><div id=tt>&nbsp;</div></td></tr>'
    tables += "</table>"
    document.write(tables)
}
...
function getWeekBE(year, month, day) {
    ...
    document.write(wkday[0] + " - " + wkday[6] + "<br>")
    for (i = 0; i < 7; i++) {
...
function printYearCal() {
    alert("test");
    document.write('<table cellspacing=0 bordercolordark=#ff00ff cellpadding=2 width=99% bordercolorlight=#999999 border=1 align="center">')
    document.write('<tr valign="middle" align="center" bgcolor="#ebebeb">')
    document.write('<td colspan="3" height="37"><img src="/images/last.gif" width="6" height="9">&nbsp;&nbsp;&nbsp;' + year + '年&nbsp;&nbsp;&nbsp;<img src="/images/next.gif" width="6" height="9">')
    document.write("</td></tr>")
    //画出每个月的表格
    for (row = 1; row <= 6; row++) {
        document.write("<tr><td width=280 height=209>")
        drawDayCal(1, getDays((row - 1) * 2 + 1, year), getcnMonthName((row - 1) * 2 + 1), year)
        document.write("</td><td width=280 height=209>")
        drawDayCal(1, getDays((row - 1) * 2 + 2, year), getcnMonthName((row - 1) * 2 + 2), year)
        document.write("</td></tr>")
