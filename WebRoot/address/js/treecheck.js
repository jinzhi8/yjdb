...
function addCheck(userid, username,obj) {
	...
	checkedObjList[checkedObjList.length] = obj.getAttribute('id');
...
function showcheckeduser(){
	...
	if (checkedIdList.length >= 0) {
		...
		if(document.getElementById('jgsxdiv')!=null){
		document.getElementById('jgsxdiv').innerHTML = "已选择<span style='color:red'>"+checkedIdList.length+"</span>个监管事项<hr/>"+htmlstr;
		document.getElementById('jgsxdiv').scrollTop = document.getElementById('jgsxdiv').scrollHeight;
		}
	}
}
