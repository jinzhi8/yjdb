...
<%
    ...
    if ("getList".equals(status)) {
        ...
        list = MyDBUtils.queryForMapToUC("select k.deptid,x.fs,x.kpid,x.id,x.bz,nvl((select o.ownername from owner o where o.id = x.userid),'系统') ownername,to_char(x.createtime,'yyyy-MM-dd') createtime from yj_kp_mx x join yj_kp k on k.id = x.kpid where k.deptid = ? and x.time = ? and x.dtype not in('2','3') order by x.createtime desc", deptid,date);
