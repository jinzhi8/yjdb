...
var webFXTreeHandler = {
    ...
    reloadNode: function (oItem) { this.all[oItem.id.replace('-cont', '')].keydown(200); },
...
WebFXTreeAbstractNode.prototype.indent = function(lvl, del, last, level, nodesLeft) {
    ...
    if (foo) {
        if ((foo._last) || ((del) && (last))) { foo.src = webFXTreeConfig.blankIcon; } else { foo.src = webFXTreeConfig.iIcon;}
    }
    return 0;
}
...
WebFXTree.prototype.getSelected = function() {
    if (webFXTreeHandler.selected) { return webFXTreeHandler.selected; } else { return null; }
}
...
WebFXTree.prototype.keydown = function(key) {
    if (key == 39) {
        if (!this.open) { this.expand(); } else if (this.childNodes.length) { this.childNodes[0].select(); }
        return false;
    }
    if (key == 37) {
        this.collapse();
...
WebFXTreeItem.prototype.getLast = function() {
    if (this.childNodes[this.childNodes.length - 1].open) { return this.childNodes[this.childNodes.length - 1].getLast(); } else { return this.childNodes[this.childNodes.length - 1]; }
}
...
WebFXTreeItem.prototype.getPreviousSibling = function(b) {
    ...
    if (i == 0) { return this.parentNode; } else {
        if ((this.parentNode.childNodes[--i].open) || (b && this.parentNode.childNodes[i].folder)) { return this.parentNode.childNodes[i].getLast(); } else { return this.parentNode.childNodes[i]; }
    }
}
// add by bxchen
function refreshNode(id) {
    ...
    if (webFXTreeHandler.onSelect) { webFXTreeHandler.onSelect(tmp); }
    webFXTreeHandler.reloadNode(tmp);
}
//end
WebFXTreeItem.prototype.keydown = function(key) {
    if ((key == 39) && (this.folder)) {
        if (!this.open) { this.expand(); } else {
            this.getFirst().select();
        }
        return false;
    } else if (key == 37) {
        if (this.open) { this.collapse(); } else {
        ...
        editNode(this.value, this.parentNode.id, this.parentNode.value);
        return false;
    } else if (key == 188) {
        upnode(this.value, this.parentNode.value, this.parentNode.id);
        return false;
    } else if (key == 190) {
        downnode(this.value, this.parentNode.value, this.parentNode.id);
        return false;
    } else if (key == 200) {
        if (this.childNodes.length) {
            this.collapse();
...
WebFXTreeItem.prototype.toString = function (nItem, nItemCount) {
    ...
    this._level = i;
    if (this.childNodes.length) { this.folder = 1; } else { this.open = false; }
    ...
    if (this.radio == "Y" || this.radion == "y" || this.radio == "true") {
        ...
        str += "<input type=\"Checkbox\" id=\"" + stext + "\" name=\"selCheckObj\" value=\"" + svalue + "\" " + selectedstr + ">";
    }

    str += "<a href=\"" + this.action + "\" id=\"" + this.id + "-anchor\" onfocus=\"webFXTreeHandler.focus(this);\" onblur=\"webFXTreeHandler.blur(this);\"" + (this.target ? " target=\"" + this.target + "\"" : "") + ">" + label + "</a></div>" + "<div id=\"" + this.id + "-cont\" class=\"webfx-tree-container\" style=\"display: " + ((this.open) ? 'block' : 'none') + ";\">";
    var sb = [];
    for (var i = 0; i < this.childNodes.length; i++) {
        sb[i] = this.childNodes[i].toString(i, this.childNodes.length);
    }
