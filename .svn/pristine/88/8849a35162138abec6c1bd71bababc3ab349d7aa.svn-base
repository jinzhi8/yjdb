...
<%	
	...
	String dunid = (String) request.getParameter("unid")==null?"":(String) request.getParameter("unid");
	...
	String sql ="select min(fkstatus) as fkstatus,min(hy) as hy,min(nextsj) as nextsj,min(qftime) as qftime,min(fkcs) as fkcs,min(qfmanid) as qfmanid,min(fkzq) as fkzq,min(fklx) as fklx,min(unid) as unid,min(year) as year,min(num) as num,min(title) as title,min(require) as require,min(leadername) as leadername,min(managedepname) as managedepname,min(source) as source,min(lxr) as lxr,min(issuetime) as issuetime,min(jbsx) as jbsx,min(qfman) as qfman,min(copyto) as copyto from (select z.*,za.nextsj,za.dbid,za.endsj,za.fkrid,decode(sign((select count(1) from ZWDBFKPG zpg where zpg.dbid=z.unid and Extract(year from zpg.fksj)='"+fkYear+"' and Extract(month from zpg.fksj)='"+fkMonth+"')),1,'已反馈','待反馈') as fkstatus,decode(z.fklx,'周期反馈',decode(z.fkzq,'每周',ceil((z.jbsx-z.qftime)/7),'半月',ceil((z.jbsx-z.qftime)/15),'每月',ceil((z.jbsx-z.qftime)/30),'半年',ceil((z.jbsx-z.qftime)/180)),1) as fkcs from zwdb z,ZWDBACL za where z.unid=za.dbid) where isjs='2' ";
	...
	//int i = 0;
		...
		row = sheet.createRow(1);
			...
			String qftime =sdft.format(sdft.parse((String)map.get("qftime")));
