...
var webFXTreeHandler = {
    ...
    reloadNode: function (oItem) {
        this.all[oItem.id.replace('-cont', '')].keydown(200);
    },
...
//得到当前的选择的值
WebFXTree.prototype.getSelectValue = function() {
    var str = "";
    try {
...
function WebFXTreeItem(sText, sAction, eParent, sIcon, sOpenIcon, sRadio, sCheckBox, sValue, sChecked, sLabel) {
    this.base = WebFXTreeAbstractNode;
    this.base(sText, sAction);
...
WebFXTreeItem.prototype._remove = function() {
    ...
    for (var i = 0; i < this.parentNode.childNodes.length; i++) {
        if (this == this.parentNode.childNodes[i]) {
            //******add 2005-05-10 by Zhang
            //当树节点收起时，记录所有已选择的子节点id

            var checkobj = document.all[this.id + "-checkbox"];
            if (checkobj != null) {
                if (checkobj.checked) {
                    addRemovedIdList(this.value);
                }
            }
...
function refreshNode(id) {
    var tmp = document.getElementById(id + '-cont');
    document.getElementById(id + '-cont').style.display = 'none';
    document.getElementById(id + '-anchor').focus();
    if (webFXTreeHandler.onSelect) {
        webFXTreeHandler.onSelect(tmp);
    }
    webFXTreeHandler.reloadNode(tmp);
}
//end
WebFXTreeItem.prototype.keydown = function(key) {
    ...
    /*
     ...
     newsubchild(this.value,this.id);
     return false ;
     }
     else if (key == 68) {
     ...
     editNode(this.value,this.parentNode.id);
     return false ;
     }
     else if (key == 188) {
     alert("up the node");
...
WebFXTreeItem.prototype.toString = function (nItem, nItemCount) {
    ...
    var label = this.text.replace(/</g, '&lt;').replace(/>/g, '&gt;')+"&nbsp;<font color=orangered>"+this.label+"</font>";
    ...
    if (this.radio == "Y" || this.radion == "y" || this.radio == "true") {
        ...
        str += "<input type=\"Checkbox\" id=\"" + this.id + "-checkbox" + "\" text=\"" + stext + "\" pid=\""+this.parentNode.id+"\" name=\"selCheckObj\" value=\"" + svalue + "\" " + selectedstr + " onclick=\"checkNode(this,this.value,this.text)\">";
		if((this.folder) &&checkCount!=1){
			selectChildStr = "<a style=\"color:orangered;\" href=\"#\" onclick=\"selectChild('"+this.id+"');\">反选</a>";
		}
    }else{
		if(checkCount!=1){
			selectChildStr = "<a style=\"color:orangered;\" href=\"#\" onclick=\"selectChild('"+this.id+"');\">反选</a>";
    ...
    str += "<a href=\"" + this.action + "\" id=\"" + this.id + "-anchor\" onfocus=\"webFXTreeHandler.focus(this);\" onblur=\"webFXTreeHandler.blur(this);\"" + (this.target ? " target=\"" + this.target + "\"" : "") + "><label for=\"" + this.id + "-checkbox" + "\">" + label + "</label></a>"+selectChildStr+"</div>" + "<div id=\"" + this.id + "-cont\" class=\"webfx-tree-container\" style=\"display: " + ((this.open) ? 'block' : 'none') + ";\">";
...
webFXTreeConfig.emptyErrorTextTemplate = "没有下一级节点！";
//webFXTreeConfig.loadErrorTextTemplate = "加载错误 \"%1%\"";
...
function WebFXLoadTree(sText, sXmlSrc, sAction, sBehavior, sIcon, sOpenIcon) {
    ...
    if (this.open)
        _startLoadXmlTree(this.src, this); else {
...
function WebFXLoadTreeItem(sText, sXmlSrc, sAction, eParent, sIcon, sOpenIcon, sRadio, sCheckBox, sValue, sChecked, sLabel) {
    // call super
...
function _xmlTreeToJsTree(oNode) {
	...
	var sLabel = oNode.getAttribute("label");

    //******add 2005-08-10 by Zhang
    //判断树节点展开时增加的子节点是否为曾经移除的子节点，若是，则从移除列表中删除，并将子节点标记为选中

    if (removeFromRemovedIdList(sValue)) {
        sChecked = "true";
    }
    ...
    var jsNode;
    if (src != null && src != "") {
        jsNode = new WebFXLoadTreeItem(text, src, action, parent, icon, openIcon, sRadio, sCheckbox, sValue, sChecked, sLabel);
    } else {
        jsNode = new WebFXTreeItem(text, action, parent, icon, openIcon, sRadio, sCheckbox, sValue, sChecked, sLabel);
    }
...
function selectChild(pid){
	var o = document.getElementsByName("selCheckObj");
	var tempCount = 0;
	for(var i=0;i<o.length;i++){
		if(o[i].getAttribute('pid')==pid){
			if(o[i].checked){
				tempCount++;
			}
		}
	}
	if(document.getElementById(pid+"-cont").style.display=='none'){
		webFXTreeHandler.toggle(document.getElementById(pid));
		Sleep(this,1000);
		this.NextStep=function(){
			for(var i=0;i<o.length;i++){
				if(o[i].getAttribute('pid')==pid){
					if(checkCount!=0&&tempCount>=checkCount){
						break;
					}
					if(o[i].checked){
						o[i].checked = false;
						checkNode(o[i],o[i].value, o[i].text);
						tempCount--;
					}
					else{
						o[i].checked = true;
