...
var setting = {
	async: {
		enable: true,
		type: "POST",
		url:"<%=request.getContextPath()%>/address/jsontree2.do",
		autoParam:["id"],
		otherParam:{type:"4"},
		dataFilter: filter
	},
...
function delChecked(checkedId){
	//alert(checkedId);
	//alert(checkedNodesId.join(",")+"  "+checkedNodesName.join(","));
	while($.inArray(checkedId,checkedNodesId)>-1){
		var delNodeIdx = $.inArray(checkedId,checkedNodesId);
		checkedNodesId.splice(delNodeIdx,1);
		checkedNodesName.splice(delNodeIdx,1);
	}
	showChecked();
...
function showChecked() {
	...
	$.each(checkedNodesName, function(key, value) {
		$("#checkedtree").append("<li>"+(key+1)+" "+value+" <a href='javascript:void(0);' onclick='delChecked(\""+checkedNodesId[key]+"\")'>X</a></li>");
	});
...
$(document).ready(function(){
	...
	/*$.post("<%=request.getContextPath()%>/address/jsontree.do",{id:"",type:"1",range:"<%=groupID%>"},function(result){
		...
		$.fn.zTree.init($("#ztree2"), setting1, result);
	...
	/*$.post("<%=request.getContextPath()%>/address/jsontree.do",{id:"",type:"1"},function(result){
		...
		$.fn.zTree.init($("#ztree1"), setting2, result);
		var zTree = $.fn.zTree.getZTreeObj("ztree1");
		var nodes = zTree.getNodes();
		for (var i=0, l=nodes.length; i<l; i++) {
			zTree.expandNode(nodes[i], true, false, false);
	...
	//全部单位
	$.post("<%=request.getContextPath()%>/address/jsontree2.do",{id:"",type:"4"},function(result){
		var setting3 = setting;
		setting3.async.otherParam.type=4;
		setting3.view.addDiyDom = function (treeId, treeNode) {
			if (treeNode.parentNode && treeNode.parentNode.label!='') return;
			var aObj = $("#" + treeNode.tId + IDMark_A);
			if (treeNode.isParent) {
		...
		$.fn.zTree.init($("#ztree1"), setting3, result);
		var zTree = $.fn.zTree.getZTreeObj("ztree1");
		var nodes = zTree.getNodes();
		for (var i=0, l=nodes.length; i<l; i++) {
			zTree.expandNode(nodes[i], true, false, false);
...
function groupcrew(){
		...
		var str_names="";
		var count=0;
		var termobj=document.getElementById("termname");
		
		$.each(checkedNodesName, function(key, value) {
			count++;
			str_ids=str_ids+checkedNodesId[key]+","
			str_names=str_names+value+",";
		});
		
		if(count>0){
			if(window.confirm("确定归组么?")){
				
				
				
			$.post("createnewgroup.jsp",{"ids":str_ids,"name":CurentTime(),"idnames":str_names},function(data){
				alert(data);
				//刷新群组
...
<div id="tabbed_box" class="tabbed_box">
	<div class="tabbed_area">
		<ul class="tabs">
			<li><a href="#" hidefocus="true" idx="content_1" class="tab active">全部单位</a></li>
			<!--<li><a href="#" hidefocus="true" idx="content_2" class="tab">本单位</a></li>-->
			<!--<li><a href="#" hidefocus="true" idx="content_3" class="tab">全部单位</a></li>-->
			<!--<li><a href="#" hidefocus="true" idx="content_4" class="tab">群组</a></li>-->
...
<div id="selected_area" class="selected_area">
	<ul class="tabs">
...
<div class="operation_area">
	...
	<input type="button" value="清  空" onclick="clear_All();getChecked();window.close();"/>&nbsp;&nbsp;
	<input type="button" value="取  消" onclick="window.close();">
	<input type="button" value="归  组"  onclick="groupcrew();">
	<input type="button" value="群组设置"  onclick="window.showModalDialog('<%=request.getContextPath()%>/personal/personalAddress.jsp',window,'status:no;dialogWidth:850px;dialogHeight:580px;scroll:yes;help:no;');window.close();">
</div>
