...
<div align="center" style="font-size:9pt">
    ...
    <input type=button value="搜索"
           onclick="document.all.searchstr2.value=document.all.searchstr.value;_find(document.getElementsByName('ck'),document.all.searchstr.value,false);">
    <input type=button value="搜索&选中"
           onclick="document.all.searchstr2.value=document.all.searchstr.value;_find(document.getElementsByName('ck'),document.all.searchstr.value,true);">
            ...
            <div style="width:100%;height:100%;overflow:auto">
                <% //用户登陆验证
					...
					for (int j = 0; j < groupList.length; j++) {
						...
						if(owner!=null){
							list = iuumService.getAllChildUserByOwnerId(owner.getId());
							if (list != null) {
								int mm=0;
								...
								for (int k = 0; k < list.size(); k++) {
									p = m + k;
									if(aclManager.isOwnerRole( ((Owner)(list.get(k))).getId(),"dc_group" )) { 
										out.print("<td align=left><input type='checkbox' style='color:orange;height:15px;' onclick='_gettotal();' name='ck' id=ck" + p + " group='"+owner.getId()+"' value='" + ((Owner) (list.get(k))).getId() + "' text=" + ((Owner) (list.get(k))).getOwnername() + " ><label for='ck" + p + "'>" + ((Owner) (list.get(k))).getOwnername() + "</label><td>");
										 mm++;
										if (mm % Integer.parseInt(getRows) == 0 && k != 0) out.print("</tr><tr>");
									}
								}
								out.print("</tr>");
								out.print("</table>");
...
<script>
	...
	function insert_group() {
		...
		if (groupUserID != "" && groupType != "" && groupID != "") {
			...
			xmlHttp.open("POST", "usergroupsact.jsp", false);
