...
public class YcInfoManager implements BaseBusiness {
    ...
    public void deleteMsss(String dbid) {
        try {
            MyDBUtils.executeUpdate("delete from yj_hy_msss where dbid=? ", dbid);
    ...
    public void insterSB(HttpServletRequest request, String unid) {
        ...
        for (int i = 1; i < num; ++i) {
            ...
            try {
                MyDBUtils.executeUpdate("insert into  yj_hy_msss(unid,dbid,psnr,sort,jtsx) values(?,?,?,?,?)", UUIDGenerator.getUUID(), unid, psnr, sort, jtsx);
            } catch (SQLException e) {
                e.printStackTrace();
            }
        }
    ...
    //民生实事插入
    public void insterMsss(MultipartRequest request, String unid) {
        String trnum = (request.getParameter("trnum") == null) ? "1" : request.getParameter("trnum");
        int num = 1;
        try {
            num = Integer.parseInt(trnum);
        ...
        for (int i = 1; i < num; ++i) {
            ...
            String jtsx = request.getParameter("jtsx" + i);
            try {
                MyDBUtils.executeUpdate("insert into  yj_hy_msss(unid,dbid,psnr,sort) values(?,?,?,?)", UUIDGenerator.getUUID(), unid, psnr, sort);
            } catch (SQLException e) {
                e.printStackTrace();
            }
        }
