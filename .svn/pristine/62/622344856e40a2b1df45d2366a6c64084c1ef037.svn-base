layui.use(['form', 'layer', 'laydate', 'table', 'laytpl', 'jquery'], function () {
    ...
    var tableIns = table.render({
        ...
        cols: [[
            ...
            {field: 'ownername', title: '反馈领导', sort: true, width: 130, align: 'center'},
            {
    ...
    table.on('tool(newsList)', function (obj) {
        ...
        if (layEvent === 'detail') { //预览
            layer.open({
                type: 2,
                area: ['100%', '100%'],
                content: getRootPatha() + '/zwdbbmcx/detail.jsp?fkid=' + data.fkid + '&rstate=' + data.rstate + '&ystate=' + data.ystate + '&gqsq=' + data.gqsq + '&bjsq=' + data.bjsq + '&unid=' + data.unid + '&deptid=' + data.tdeptid + '&isld=1',
            });
        }
    });
