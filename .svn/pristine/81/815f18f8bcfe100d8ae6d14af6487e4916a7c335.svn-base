...
<%
	 ...
	 int count=MyDBUtils.queryForInt("select count(1) from common_attachment t where docunid='附件上传'  order by uploadtime desc");
	 List<Map<String,Object>> list=MyDBUtils.queryForMapToUC("select t.*,to_char(t.uploadtime,'yyyy-mm-dd')time  from common_attachment t where docunid='附件上传'  order by uploadtime desc");
%>
<!DOCTYPE html>
<html>
<head>
  ...
  <div class="fly-panel fly-panel-user newclass-fly-panel">
    <div class="layui-tab layui-tab-brief" lay-filter="user">
      <ul class="layui-tab-title">
        <li class="layui-this">资料下载（<span><%=count%></span>）</li>
        <%if(aclManager.isOwnerRole(userID, "dbk")){%>
        	<li class="addNews_btn">资料上传</li>
        <%}%>
      </ul>
