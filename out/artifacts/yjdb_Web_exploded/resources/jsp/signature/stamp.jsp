...
<script>
	...
	function checkPhoto(fnUpload)
		...
		if((mime!=".jpg")&&(mime!=".bmp")&&(mime!=".jpeg")&&(mime!=".png"))
		{
		alert("请选择jpg、bmp、jpeg、png格式的照片上传");
	...
	function upload(file){
		 ...
		 var mime= filename.toLowerCase().substr(filename.lastIndexOf("."));
		 if((mime!=".jpg")&&(mime!=".pdf"))
         {
		 alert("请选择jpg、pdf格式的照片上传");
	...
	function showZxsczm(){
		var retval=window.showModalDialog('<%=request.getContextPath()%>/resources/jsp/signature/pdfqz.jsp?act=sczm&docid=<%=docid%>&moduleId=<%=moduleId%>',window,'status:no;dialogWidth:980px;dialogHeight:630px;scroll:no;help:no;');
	...
	function showBdqz(){
		var retval=window.showModalDialog('<%=request.getContextPath()%>/resources/jsp/signature/pdfqz.jsp?docid=<%=docid%>&moduleId=<%=moduleId%>',window,'status:no;dialogWidth:980px;dialogHeight:630px;scroll:no;help:no;');
		//alert("showBdqz"+retval);
		if(retval=='1'){
			closeQz();
			window.location.href=window.location.href;
	...
	function loadPdf(uuid,path){
		var retval=window.showModalDialog('<%=request.getContextPath()%>/resources/jsp/signature/pdfqz.jsp?act=transfor&docid=<%=docid%>&moduleId=<%=moduleId%>&uuid='+uuid,window,'status:no;dialogWidth:980px;dialogHeight:630px;scroll:no;help:no;');
...
<div id="divQz" class="white_content" style="width:250px;height:200px">
  ...
  <div style="text-align:left;padding:0 0 0 30px;font-size:14px;margin-top:15px;">
	 <!--  <p><a href="javascript:void(0)" onclick="showZxsczm();" style="line-height:24px;">1、自动生成证明</a></p>
	 <p><a href="javascript:void(0)" onclick="showZxqz();" style="line-height:24px;">2、申请人上传证明</a></p> -->
	  <p><a href="javascript:void(0)" onclick="showUp();" style="line-height:24px;">纸质盖章证明扫描件</a></p>
	<!--   <p><a href="javascript:void(0)" onclick="showBdqz();" style="line-height:24px;">4、纸质未盖章证明扫描件</a></p> -->
  </div>
</div>
...
<div id="divUp" class="white_content" style="width:300px;height:200px">
    ...
    <div style="text-align:center;padding:0 0 0 30px;font-size:14px;margin-top:15px;">
		<form id='fileForm' enctype='multipart/form-data'>
			 ...
			 <br/>
			 <input type="button" onclick="upload(file)" value="上传">&nbsp;&nbsp;<input type="button" onclick="closeUp()" value="取消">
		 </form>
    </div>
</div>
