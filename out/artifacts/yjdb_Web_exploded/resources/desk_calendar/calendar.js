...
pos = urlstr.indexOf("&yea=");
urlstr = urlstr.substring(pos + 5, urlstr.length);

var pos = urlstr.indexOf("&mon=");
var year = urlstr.substring(0, pos);

urlstr = urlstr.substring(pos + 5, urlstr.length);
pos = urlstr.indexOf("&day=");

var month = urlstr.substring(0, pos);
var day = urlstr.substring(pos + 5, urlstr.length);
...
var gdCurDate = new Date(year, month - 1, day);
...
function fSetDate(iYear, iMonth, iDay) {
    ...
    // window.close();
    //alert(gdCtrl.value);
    window.parent.gzap.location = "../../arrange/desk_arrange.jsp?yea=" + iYear + "&mon=" + iMonth + "&dat=" + iDay;
}
...
with (document) {
    ...
    write("<table border='0' bgcolor='#c0c0c0'>");
    write("<TR bgcolor='#c0c0c0'>");
    write("<td valign='middle' align='center'><input type='button' name='PrevMonth' value='<' style='height:23;width:18;FONT:bold' onClick='fPrevMonth()'>");
    write("&nbsp;<SELECT name='tbSelYear' onChange='fUpdateCal(tbSelYear.value, tbSelMonth.value)' Victor='Won'>");

    var today = new Date(year, month - 1, day); //取服务器年月日

    //var today = new Date();
    var thisYear = today.getFullYear();
    var thisYear10 = thisYear + 10;
    var thisYear_30 = thisYear - 5;
    //计算从今年后的10年历和以前的20年…………

    for (i = thisYear_30; i < thisYear10; i++)
    ...
    write("</SELECT>");
    write("&nbsp;<input type='button' name='PrevMonth' value='>' style='height:23;width:18;FONT:bold' onclick='fNextMonth()'>");
    write("</td>");
    ...
    write("</table></DIV>");
    write("</td>");
    write("</TR><TR  bgcolor='#c0c0c0'><TD align='center'>");
    write("<TABLE width='100%' bgcolor='#c0c0c0'><TR><TD align='center'>");
    write("<B ID=\"CAL_B_BLANK\" ></B>");
    ...
    write("<B style='color:" + gcBG + ";cursor:hand; font-size:12px' onclick='fSetDate(giYear,giMonth,giDay)'>今天: <span onMouseOver='this.style.color=orange' onMouseOut='this.style.color=gcBG'>" + giYear + "-" + giMonth + "-" + giDay + "</span></B>");
    write("</td></tr></table>");
    write("</TD></TR>");
    write("</TABLE>");
    write("</Div>");
