...
<template:put name='title' content='个性化默认定制' direct='true'/>
...
<% //用户登陆验证
	...
	if (request.getParameter("depflag") != null && request.getParameter("depflag") != "") {
		userflag = request.getParameter("depflag");
	}
...
<% //post就保存
	if (request.getMethod().equalsIgnoreCase("post")) {
		...
		if (action == null || action.length() == 0) {
			...
			ManageTemplateBean.saveDefaultContent(userflag, leftcol, midcol, rightcol, "5");
		}
	}%>
<% Connection infoconn = null;
	...
	try {
		...
		//查询所有布局
		String SelectPERSONLAYOUT = "select p.P_CONTENT,p.P_LAYOUT_TYPE,f.CAPTION CAPTION,p.P_DEPFLAG from PERSONDEFAULTLAYOUT p,TOPICS_FUNC f  where p.P_CONTENT=f.ID and p.P_DEPFLAG=? order by P_LAYOUT_TYPE,P_ORDER";
		stat = infoconn.prepareStatement(SelectPERSONLAYOUT);
		//debug
		stat.setString(1, userflag);
...
<table width="752" border="0" align="center" cellpadding="0" cellspacing="0">
	<tr>
		...
		<td width="75"><img src="images/top_02.gif" width="75" height="24"></td>
...
<table width="752" border=0 cellpadding=4 cellspacing=0 class="little_text_0" style="border:solid 1pt #C01010;">
	<tr>
		<td align=left colspan="2">
			请改变栏目布局。选择您想移动的内容，左、右栏中的内容对应您页面上的左、右侧，点击<img src="images/up.gif" width="16" height="16" border="0">向上移，点击<img src="images/dn.gif" width="16" height="16" border="0">下移，点击<img src="images/x.gif" width="16" height="16" border="0">删除内容。完成后，点击"保存"。
		</td>
	</tr>
</table>
<br>
<table width="752" border=0 cellpadding=4 cellspacing=0 class="little_text_0" style="border:solid 1pt #C01010;">
	<tr>
		<td align=left colspan="2" class="tdbottom">以下栏目内容可以加入到上面左、右二栏中。
			先选择下面的栏目，再选择放入左、右哪一栏，最后点确定就可以加入想在个性化页面中显示的栏目。调整好所有布局后按“保存”按钮进行保存。
		</td>
	</tr>
</table>
<br>
	...
	<tr align="center">
		...
		<td valign="top" align="left">
			<table width="200" border="0" cellspacing="0" cellpadding="0" class="round">
				<tr align="center">
					<td height="30" colspan="2" class="title2">左 栏</td>
				</tr>
				<tr>
		...
		<td valign="top" align="right">
			<table width="200" border="0" cellspacing="0" cellpadding="0" class="round">
				<tr align="center">
					<td height="30" colspan="2" class="title2">右 栏</td>
				</tr>
				<tr>
...
<table width="752" border=0 cellpadding=4 cellspacing=0 class="little_text_0" style="border:solid 1pt #C01010;">
	<tr>
		<td align=center colspan="1">用户类别：<select name="depflag" style="width:100" onchange="selectdep(this)">
			<option value="1">办公用户</option>
			<option value="0">部门用户</option>
		</select>&nbsp;&nbsp;<a href="javascript:postsave();" class="formbutton">保 存</a>
		</td>
	</tr>
</table>
<br>
...
<script language="JavaScript">
	disablecol();
	for (var j = 0; j < document.all.depflag.options.length; j++) {
		if (document.all.depflag.options[j].value == "<%=userflag%>") {
			document.all.depflag.selectedIndex = j;
		}
	}
	function selectdep(obj) {
		location.href = "changedefault.jsp?depflag=" + obj.value;
	}
</script>
