...
<%
	...
	FlowTransmitInfo flowTransInfo = (FlowTransmitInfo) request.getAttribute("flowTransmitInfo");
	Activity curActivity = flowTransInfo.getCurActivity();
	String ders = (String)curActivity.getDescription();
    ...
    function remove(count){
        ...
        function openNew(zunid,docunid)
            {   
                ...
                var win = window.open("<%=request.getContextPath()%>/zw_hydb/openContent.jsp?unid="+zunid+"&docunid=<%=docUNID%>&ld="+leadername, "弹出窗口", "width=" + width + ", height=" + height + ",top=" + iTop + ",left=" + iLeft + ",toolbar=no, menubar=no, scrollbars=yes, resizable=no,location=no, status=no,alwaysRaised=yes,depended=yes");
            }
...
<form action="<%=request.getContextPath()%>/save" method="post" enctype="multipart/form-data" onSubmit="return validateContext();">
    ...
    <table border="0" align="center" cellpadding="0" cellspacing="0" class="round">
        ...
        <table id="baseinfo" width="99%" border="0" align="center" cellpadding="0" cellspacing="0" class="table">
            ...
            <tr valign="top">
                ...
                <td class="tinttd" >
			        ...
			        <img style="cursor:default;" title="点击选择处理人" src="<%=request.getContextPath()%>/resources/images/actn133.gif" onclick="openSelWin('<%=request.getContextPath()%>/address/tree_ry.jsp?utype=1&sflag=0&count=1&fields=leadername,leaderid');">
             ...
             <tr valign="top">
                ...
                <td class="tinttd" >
                    <html:select name = "source" style="width:150px">
                        <html:option value = "县长例会">县长例会</html:option>
                        <html:option value = "县政府全体会议">县政府全体会议</html:option>
                        <html:option value = "县政府常务会议">县政府常务会议</html:option>
                        <html:option value = "县长办公会议">县长办公会议</html:option>
                        <html:option value = "县长（主任）例会">县长（主任）例会</html:option>
                        <html:option value = "县政府专题会议">县政府专题会议</html:option>
        ...
        <tr>
            <td height="30">
                <DIV align="center">
                        <input type="submit" name="submitContent" class="layui-btn" value="提  交" onclick="if(validateForm()){setSubmitMethod('2');return checkperformer(submitFlow());}else{return false;}"/>
                        <input type="submit" name="finishContent" class="layui-btn layui-btn-normal" value="同意督办" onclick="if(validateForm()){isjsls();tydb();insertRange();setSubmitMethod('4');submitFlow();}else{return false;}"/>
                        <input type="submit" name="dbfinishContent" class="layui-btn layui-btn-normal" value="不同意督办" onclick="if(validateForm()){btysfdb();setSubmitMethod('4');submitFlow();}else{return false;}"/>
                        <!--<input type="submit" name="delContent" class="formbutton" value="删  除" onclick="setSubmitMethod('9');document.all.xmlType.value='delete';return confirmDelete();"/>
                        <a class="viewbutton" target="_blank" href="<%=request.getContextPath()%>/lwzwjb/dytzs.jsp?id="><span>打印政务交办通知书</span></a>   -->
                </DIV>
            </td>
        </tr>
    </table>
